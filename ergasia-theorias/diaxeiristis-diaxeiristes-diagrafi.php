...
if(isset($_POST["id_diaxeiristi"]) && !empty($_POST["id_diaxeiristi"])){
    
    require_once "syndesh-geniki.php";
    
	mysqli_query($link,"delete from omadaxrhsth where username in (select omadaxrhsth.username from diaxeiristis,omadaxrhsth,omadaxrhsth_me_diaxeiristi where diaxeiristis.id_diaxeiristi=omadaxrhsth_me_diaxeiristi.id_diaxeiristi and omadaxrhsth.username=omadaxrhsth_me_diaxeiristi.username and diaxeiristis.id_diaxeiristi='" . $_POST['id_diaxeiristi'] . "')");
	mysqli_query($link,"DELETE FROM diaxeiristis where id_diaxeiristi='" . $_POST['id_diaxeiristi'] . "'");
	header("location: diaxeiristis-diaxeiristes.php");
    mysqli_close($link);
} else{
  
    if(empty(trim($_GET["id_diaxeiristi"]))){
       
        header("location: diaxeiristis-diaxeiristes-diagrafi.php");
        exit();
    }
}
?>
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="alert alert-danger">
                            <input type="hidden" name="id_diaxeiristi" value="<?php echo trim($_GET["id_diaxeiristi"]); ?>"/>
                            ...
                            <p>
                                ...
                                <a href="diaxeiristis-diaxeiristes.php" class="btn btn-secondary">Όχι</a>
                            </p>
                        </div>
                    </form>
                </div>
