...
include "diaxeiristis-elegxos.php";
 ...
 if (isset($_POST['submit'])) {
	 ...
	 $id_diaxeiristii=$_POST['id_diaxeiristi'];
$email_diaxeiristii=$_POST['email_diaxeiristi'];
$usernamee=$_POST['username'];
$resultset_1 = mysqli_query($link, "select * from diaxeiristis,omadaxrhsth where (id_diaxeiristi='".$id_diaxeiristii."'  or email_diaxeiristi='".$email_diaxeiristii."' or username='".$usernamee."');")  or die(mysql_error());
$count = mysqli_num_rows($resultset_1);


if($count == 0){
		
$stmt1 = $link->prepare("INSERT INTO diaxeiristis (id_diaxeiristi, email_diaxeiristi, onoma_diaxeiristi, eponimo_diaxeiristi) VALUES (?, ?, ?, ?)");
$id_diaxeiristi=$_POST['id_diaxeiristi'];
$email_diaxeiristi=$_POST['email_diaxeiristi'];
$onoma_diaxeiristi=$_POST['onoma_diaxeiristi'];
$eponimo_diaxeiristi=$_POST['eponimo_diaxeiristi'];
$stmt1->bind_param("ssss", $id_diaxeiristi, $email_diaxeiristi, $onoma_diaxeiristi, $eponimo_diaxeiristi);
$stmt1->execute();
...
$onoma_omadaxrhsth='ΔΙΑΧΕΙΡΙΣΤΗΣ';
$stmt2->bind_param("sss", $username, $hashed_password, $onoma_omadaxrhsth);
$stmt2->execute();


$stmt3 = $link->prepare("INSERT INTO omadaxrhsth_me_diaxeiristi (id_diaxeiristi, username) VALUES (?, ?)");
$id_diaxeiristi=$_POST['id_diaxeiristi'];
$username=$_POST['username'];
$stmt3->bind_param("ss", $id_diaxeiristi, $username);
$stmt3->execute();

	header("Location: diaxeiristis-diaxeiristes.php");
...
mysqli_stmt_close($stmt3);
	...
	header("Location: diaxeiristis-diaxeiristes-prosthiki.php");
...
<div class="form-group">
            <input type="text" name="id_diaxeiristi" class="form-control" placeholder="ID Διαχειριστή" required="required">	
        </div>

        <div class="form-group">
            <input type="text" name="email_diaxeiristi" class="form-control" placeholder="E-mail Διαχειριστή" required="required">	
        </div>
		
		<div class="form-group">
            <input type="text" name="onoma_diaxeiristi" class="form-control" placeholder="Όνομα Διαχειριστή" required="required">	
        </div>
		
		<div class="form-group">
            <input type="text" name="eponimo_diaxeiristi" class="form-control" placeholder="Επώνυμο Διαχειριστή" required="required">	
        </div>
		...
		<div class="form-group">
            <input type="text" class="form-control" placeholder="ΔΙΑΧΕΙΡΙΣΤΗΣ" disabled>	
        </div>
