...
if(isset($_POST["id_mathimatos"]) && !empty($_POST["id_mathimatos"])){
    
    require_once "syndesh-geniki.php";
    mysqli_query($link,"DELETE FROM mathima where id_mathimatos='" . $_POST['id_mathimatos'] . "'");
	header("location: diaxeiristis-mathimata.php");
    mysqli_close($link);
} else{
  
    if(empty(trim($_GET["id_mathimatos"]))){
       
        header("location: diaxeiristis-mathimata-diagrafi.php");
        exit();
    }
}
?>
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="alert alert-danger">
                            <input type="hidden" name="id_mathimatos" value="<?php echo trim($_GET["id_mathimatos"]); ?>"/>
                            <p>Θέλετε να διαγράψετε αυτό το μάθημα; Η ενέργεια είναι μη αναστρέψιμη.</p>
                            <p>
                                <input type="submit" value="Ναι" class="btn btn-danger" >
                                <a href="diaxeiristis-mathimata.php" class="btn btn-secondary">Όχι</a>
                            </p>
                        </div>
                    </form>
                </div>
