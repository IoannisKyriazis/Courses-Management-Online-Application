...
$email_foititii=$_POST['email_foititi'];
$usernamee=$_POST['username'];
$resultset_1 = mysqli_query($link, "select * from foititis,omadaxrhsth where (AM_foititi='".$AM_foititii."'  or email_foititi='".$email_foititii."' or username='".$usernamee."');")  or die(mysql_error());
$count = mysqli_num_rows($resultset_1);


if($count == 0){
		
$stmt1 = $link->prepare("INSERT INTO foititis (AM_foititi, email_foititi, onoma_foititi, eponimo_foititi, etoseisagogis) VALUES (?, ?, ?, ?, ?)");
$AM_foititi=$_POST['AM_foititi'];
$email_foititi=$_POST['email_foititi'];
$onoma_foititi=$_POST['onoma_foititi'];
$eponimo_foititi=$_POST['eponimo_foititi'];
$etoseisagogis=$_POST['etoseisagogis'];
$stmt1->bind_param("sssss", $AM_foititi, $email_foititi, $onoma_foititi, $eponimo_foititi, $etoseisagogis);
$stmt1->execute();
...
$onoma_omadaxrhsth='ΦΟΙΤΗΤΗΣ';
$stmt2->bind_param("sss", $username, $hashed_password, $onoma_omadaxrhsth);
$stmt2->execute();


$stmt3 = $link->prepare("INSERT INTO omadaxrhsth_me_foititi (AM_foititi, username) VALUES (?, ?)");
$AM_foititi=$_POST['AM_foititi'];
$username=$_POST['username'];
$stmt3->bind_param("ss", $AM_foititi, $username);
$stmt3->execute();

	header("Location: diaxeiristis-foitites.php");
...
mysqli_stmt_close($stmt3);
	...
	header("Location: diaxeiristis-foitites-prosthiki.php");
...
<div class="container-fluid">
	<h3 class="mt-4">Προσθήκη Φοιτητή</h3>
...
<div class="form-group">
            <input type="text" name="AM_foititi" class="form-control" placeholder="Αριθμός Μητρώου" required="required">	
        </div>

        <div class="form-group">
            <input type="text" name="email_foititi" class="form-control" placeholder="E-mail Φοιτητή" required="required">	
        </div>
		
		<div class="form-group">
            <input type="text" name="onoma_foititi" class="form-control" placeholder="Όνομα Φοιτητή" required="required">	
        </div>
		
		<div class="form-group">
            <input type="text" name="eponimo_foititi" class="form-control" placeholder="Επώνυμο Φοιτητή" required="required">	
        </div>
		
		<div class="form-group">
            <input type="text" name="etoseisagogis" class="form-control" placeholder="Έτος Εισαγωγής" required="required">	
        </div>
		...
		<div class="form-group">
            <input type="text" class="form-control" placeholder="ΦΟΙΤΗΤΗΣ" disabled>	
        </div>
