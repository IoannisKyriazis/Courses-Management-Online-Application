...
if(isset($_POST["id_didaskonta"]) && !empty($_POST["id_didaskonta"])){
    
    require_once "syndesh-geniki.php";
    
	mysqli_query($link,"delete from omadaxrhsth where username in (select omadaxrhsth.username from didaskon,omadaxrhsth,omadaxrhsth_me_didaskonta where didaskon.id_didaskonta=omadaxrhsth_me_didaskonta.id_didaskonta and omadaxrhsth.username=omadaxrhsth_me_didaskonta.username and didaskon.id_didaskonta='" . $_POST['id_didaskonta'] . "')");
	mysqli_query($link,"DELETE FROM didaskon where id_didaskonta='" . $_POST['id_didaskonta'] . "'");
	header("location: diaxeiristis-didaskontes.php");
    mysqli_close($link);
} else{
  
    if(empty(trim($_GET["id_didaskonta"]))){
       
        header("location: diaxeiristis-didaskontes-diagrafi.php");
        exit();
    }
}
?>
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="alert alert-danger">
                            <input type="hidden" name="id_didaskonta" value="<?php echo trim($_GET["id_didaskonta"]); ?>"/>
                            ...
                            <p>
                                ...
                                <a href="diaxeiristis-didaskontes.php" class="btn btn-secondary">Όχι</a>
                            </p>
                        </div>
                    </form>
                </div>
