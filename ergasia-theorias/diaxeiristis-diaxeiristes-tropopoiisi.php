...
include "diaxeiristis-elegxos.php";
 include "syndesh-geniki.php";
	...
	if(count($_POST)>0) {
mysqli_query($link,"UPDATE diaxeiristis, omadaxrhsth_me_diaxeiristi set diaxeiristis.onoma_diaxeiristi='" . $_POST['onoma_diaxeiristi'] . "', diaxeiristis.eponimo_diaxeiristi='" . $_POST['eponimo_diaxeiristi'] . "', omadaxrhsth_me_diaxeiristi.onoma_omadaxrhsth='" . $_POST['onoma_omadaxrhsth'] . "', omadaxrhsth_me_diaxeiristi.password='" . $_POST['password'] . "' WHERE id_diaxeiristi='" . $_GET['id_diaxeiristi'] . "' and diaxeiristis.id_diaxeiristi=omadaxrhsth_me_diaxeiristi.id_diaxeiristi and omadaxrhsth.username=omadaxrhsth_me_diaxeiristi.username");

mysqli_query($link,"UPDATE omadaxrhsth set onoma_omadaxrhsth='" . $_POST['onoma_omadaxrhsth'] . "', password='" . $_POST['password'] . "' WHERE username in (select omadaxrhsth.username from diaxeiristis, omadaxrhsth, omadaxrhsth_me_diaxeiristi where diaxeiristis.id_diaxeiristi='" . $_GET['id_diaxeiristi'] . "' and omadaxrhsth.username=omadaxrhsth_me_diaxeiristi.username and diaxeiristis.id_diaxeiristi=omadaxrhsth_me_diaxeiristi.id_diaxeiristi)");
...
$result = mysqli_query($link,"select * from diaxeiristis, omadaxrhsth, omadaxrhsth_me_diaxeiristi where diaxeiristis.id_diaxeiristi='" . $_GET['id_diaxeiristi'] . "' and omadaxrhsth.username=omadaxrhsth_me_diaxeiristi.username and diaxeiristis.id_diaxeiristi=omadaxrhsth_me_diaxeiristi.id_diaxeiristi");
...
<nav class="nav text-dark bg-light py-2" name="navigation-menu">
	<ul class="nav pl-4">
		...
		<li class="nav-item">
			<a href="diaxeiristis-diaxeiristes.php" class="text-muted text-decoration-none small">Διαχειριστές</a>
		</li>
...
<div class="container-fluid">
	<h3 class="mt-4">Τροποποίηση διαχειριστή</h3>
	...
	<div class="container">
                    ...
                    <div class="card shadow-lg border-0 rounded-lg mt-5">
                        <div class="card-header"><h3 class="text-center font-weight-light my-4">Στοιχεία</h3></div>
							...
							<form name="frmUser" method="post" action="">
									...
									<input type="text" name="id_diaxeiristi" class="form-control" value="<?php echo $row['id_diaxeiristi']; ?>" disabled>
                                    ...
                                    <input type="text" name="onoma_diaxeiristi" class="form-control" value="<?php echo $row['onoma_diaxeiristi']; ?>" >
									
									<div class="input-group-prepend mt-5"><span class="input-group-text">Επώνυμο</span></div>
                                    <input type="text" name="eponimo_diaxeiristi" class="form-control" value="<?php echo $row['eponimo_diaxeiristi']; ?>" >
									
									<div class="input-group-prepend mt-5"><span class="input-group-text">E-mail</span></div>
                                    <input type="text" name="email_diaxeiristi" class="form-control" value="<?php echo $row['email_diaxeiristi']; ?>" disabled>
			...
			<a href="diaxeiristis-diaxeiristes.php" class="btn btn-secondary active" role="button" aria-pressed="true">Ακύρωση / ΠΙΣΩ</a>
        </div>
